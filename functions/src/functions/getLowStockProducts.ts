...
function parseCursor(value: unknown): SummaryListCursor | null {
  ...
  const cursor = value as { updatedAtMs?: unknown; docId?: unknown };
...
function isLowOrOut(item: ProductSummaryListItem, outOnly: boolean): boolean {
  if (outOnly) {
...
export const getLowStockProducts = onCall(
  async (request): Promise<GetLowStockProductsResult> => {
    ...
    const outOnly = parseBoolean(request.data?.outOnly);
    ...
    const snapshot = await query.limit(limit + 25).get();
    ...
    const filteredDocs = snapshot.docs.filter((doc) => {
      ...
      return isLowOrOut(item, outOnly) && matchesQuery(item, queryText);
    });
